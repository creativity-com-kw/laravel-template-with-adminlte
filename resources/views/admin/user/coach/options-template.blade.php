<div class="btn-group">
    ...
    <ul class="dropdown-menu dropdown-menu-right">
        <li><a class="dropdown-item" href="{{ route('admin.users.coaches.edit', [$user]) }}">Edit</a></li>
        @if(!$user->trashed())
...
<form method="POST" action="{{ route('admin.users.coaches.destroy', [$user]) }}" id="delete-form-{{ $user->id }}" style="display: none;">
    @csrf
    @method('DELETE')
</form>

<form method="POST" action="{{ route('admin.users.coaches.restore', [$user]) }}" id="restore-form-{{ $user->id }}" style="display: none;">
    @csrf
    @method('PUT')
</form>
