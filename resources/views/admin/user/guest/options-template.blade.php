<div class="btn-group">
    ...
    <ul class="dropdown-menu dropdown-menu-right">
        <li><a class="dropdown-item" href="{{ route('admin.users.guests.edit', [$user]) }}">Edit</a></li>
        @if(!$user->trashed())
...
<form method="POST" action="{{ route('admin.users.guests.destroy', [$user]) }}" id="delete-form-{{ $user->id }}" style="display: none;">
    @csrf
    @method('DELETE')
</form>

<form method="POST" action="{{ route('admin.users.guests.restore', [$user]) }}" id="restore-form-{{ $user->id }}" style="display: none;">
    @csrf
    @method('PUT')
</form>
